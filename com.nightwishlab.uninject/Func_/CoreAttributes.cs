...
{
    /// <summary>
    /// true면 해당 의존성이 없어도 경고/실패로 취급하지 않습니다.
...
/// <summary>
/// 주입이 완료된 시점에 후처리를 수행하고 싶은 컴포넌트를 위한 콜백 인터페이스입니다.
/// - ObjectInstaller.TryInjectTarget()가 "필수 의존성" 주입에 성공했을 때 호출됩니다.
/// - Optional 의존성 누락은 성공/실패에 영향을 주지 않습니다.
/// </summary>
...
{
    /// <summary>
    /// 이 컴포넌트를 어떤 타입(주로 인터페이스)으로 바인딩할지 지정합니다.
    /// 지정되면 MasterInstaller 레지스트리는 구체 타입 대신 BindType 을 Key 로 사용합니다.
    /// </summary>
...
public class SceneReferralAttribute : Attribute
{
    /// <summary>
    /// 이 컴포넌트를 어떤 타입(주로 인터페이스)으로 바인딩할지 지정합니다.
    /// 지정되면 SceneInstaller 레지스트리는 구체 타입 대신 BindType 을 Key 로 사용합니다.
    /// </summary>
    public Type BindType { get; private set; }
